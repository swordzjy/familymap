...
--
-- 使用说明：
-- 1. 创建数据库：createdb -U postgres family_migration
-- 2. 执行此脚本：psql -U postgres -d family_migration -f deploy.sql
-- ============================================================
...
CREATE TABLE IF NOT EXISTS places (
  ...
  geocode_source  VARCHAR(20) DEFAULT 'amap',
  created_at      TIMESTAMPTZ DEFAULT NOW(),
  CONSTRAINT places_raw_name_unique UNIQUE(raw_name)
);

CREATE INDEX IF NOT EXISTS idx_places_raw_name ON places USING GIN (raw_name gin_trgm_ops);
...
CREATE TABLE IF NOT EXISTS migrations (
  ...
  from_place_raw  VARCHAR(200),
  to_place_raw    VARCHAR(200),
  year            INTEGER,
  year_approx     BOOLEAN DEFAULT FALSE,
  reason          TEXT,
  reason_type     VARCHAR(30),
  emotion_weight  VARCHAR(10) DEFAULT 'medium',
  sequence_order  INTEGER,
  created_at      TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_migrations_family ON migrations(family_id);
...
DROP TRIGGER IF EXISTS trg_family_profiles_updated_at ON family_profiles;
...
-- 视图：完整的家族迁徙路径
-- ============================================================
CREATE OR REPLACE VIEW v_migration_paths AS
SELECT
  m.id,
  m.family_id,
  m.sequence_order,
...
-- 预置数据：历史事件
-- ============================================================
INSERT INTO historical_events (name, year_start, year_end, region, event_type, emotion_tone, description)
SELECT * FROM (VALUES
...
) AS t(name, year_start, year_end, region, event_type, emotion_tone, description)
WHERE NOT EXISTS (SELECT 1 FROM historical_events WHERE name = t.name);
